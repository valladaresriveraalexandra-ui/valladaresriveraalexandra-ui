...
class ConsultaHistorial(tk.Frame):
    ...
    # -------------------- FUNCIÓN AUXILIAR PARA REPORTE INDIVIDUAL --------------------
    def _generar_reporte_individual(self, est_id, nombre_completo, grado, movimientos):
        """Genera el contenido de un reporte para un solo estudiante."""
        lineas = []
        lineas.append("=" * 70)
    ...
    # -------------------- FUNCIÓN PRINCIPAL DE IMPRESIÓN (MODIFICADA) --------------------
    def _imprimir_registro_estudiante(self):
        ...
        # --- Caso 1: Hay selección -> generamos un único archivo con TODOS los reportes individuales ---
        if seleccion:
            contenido_total = []
            ...
            # Abrir el archivo en el Bloc de notas / editor predeterminado
            try:
                os.startfile(ruta)
                mostrar_toast(
                    self.winfo_toplevel(),
                    f"📄 Reporte combinado abierto ({len(ids_estudiantes)} estudiante(s) seleccionados)",
                    color=COLOR_BLUE,
                    duracion=4000
                )
        ...
        # --- Caso 2: Sin selección -> reporte consolidado (comportamiento original) ---
        else:
            lineas = []
            lineas.append("=" * 70)
            lineas.append("REGISTRO DE ASISTENCIA ESTUDIANTIL (RAE)")
            ...
            try:
                ...
                mostrar_toast(
                    self.winfo_toplevel(),
                    f"📄 Reporte consolidado abierto ({len(ids_estudiantes)} estudiante(s))",
                    color=COLOR_BLUE,
                    duracion=3000
                )
            except AttributeError:
