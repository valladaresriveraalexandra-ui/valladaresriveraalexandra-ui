...
# ═══════════════════════════════════════════════════════════════
# NAVBAR — tres variantes:
#   make_navbar        → gris, para ventanas internas (Volver)
#   make_navbar_panel  → AZUL, para portería
#   make_navbar_admin  → AZUL CLARO, para administrador
# ═══════════════════════════════════════════════════════════════
def make_navbar(parent, items):
    """Navbar gris para ventanas internas (botón Volver, etc.)."""
...
def make_navbar_panel(parent, items):
    """Navbar AZUL para el panel de PORTERÍA."""
    COLOR_NAV    = "#1a73e8"
    COLOR_NAV_HV = "#1558b0"
...
def make_navbar_admin(parent, items):
    """Navbar AZUL CLARO para el panel de ADMINISTRACIÓN."""
    COLOR_NAV    = "#1a73e8"   # azul principal
    COLOR_NAV_HV = "#1558b0"   # azul más oscuro al hover
    # Color especial para el botón "Inicio" → negro
    COLOR_INICIO    = "#1a1a1a"
...
def realizar_registro_db(est_id, nombre, apellido, master=None):
    ...
    if master:
        ...
        mensaje = f"{emoji}  {nombre} {apellido}  —  {tipo} registrado"
        mostrar_toast(master, mensaje, color=color, duracion=3000)
    return tipo, nombre, apellido
...
class PanelPorteria(tk.Frame):
    ...
    def _build(self):
        ...
        cols = ("Estudiante", "Fecha", "Hora Entrada", "Hora Salida", "Estado")
        self.tree = ttk.Treeview(card, columns=cols, show="headings", height=16)
        widths  = {"Estudiante": 320, "Fecha": 160,
                   "Hora Entrada": 160, "Hora Salida": 160, "Estado": 180}
        anchors = {"Estudiante": "w", "Fecha": "center",
                   "Hora Entrada": "center", "Hora Salida": "center", "Estado": "center"}
        for col in cols:
            ...
            for tipo, hora, reg_id in movs:
                if tipo == "Ingreso":
                    if pendiente_ingreso is not None:
                        filas.append((nombre_est, fch, pendiente_ingreso, "—", "dentro", est_id, pendiente_id))
                    pendiente_ingreso = hora
                    pendiente_id = reg_id
                elif tipo == "Salida":
                    if pendiente_ingreso is not None:
                        filas.append((nombre_est, fch, pendiente_ingreso, hora, "fuera", est_id, reg_id))
                        pendiente_ingreso = None
                        pendiente_id = None
                    else:
                        filas.append((nombre_est, fch, "—", hora, "fuera", est_id, reg_id))
            if pendiente_ingreso is not None:
                filas.append((nombre_est, fch, pendiente_ingreso, "—", "dentro", est_id, pendiente_id))

        filas.sort(key=lambda x: (x[1], x[2]), reverse=True)

        for idx, (nombre_est, fch, h_entrada, h_salida, tag, est_id, last_reg_id) in enumerate(filas):
            estado_txt = "✅ Dentro" if tag == "dentro" else "🚪 Salió"
            self.tree.insert("", "end",
                             values=(nombre_est, fch, h_entrada, h_salida, estado_txt),
                             tags=(tag,))
...
# ═══════════════════════════════════════════════════════════════
#  PANEL ADMINISTRACIÓN — navbar AZUL
# ═══════════════════════════════════════════════════════════════
class PanelAdministracion(tk.Frame):
    ...
    def _build(self):
        make_header(self, "RAE")
        # Navbar AZUL para administrador (botón Inicio en negro)
...
#  ID mostrado es secuencial (1,2,3...) — ID real guardado en _real_ids
# ═══════════════════════════════════════════════════════════════
class GestionEstudiantes(tk.Frame):
    def __init__(self, master, root=None, close_callback=None):
        ...
        self.close_callback = close_callback
        self._search_after_id = None
        self._real_ids = {}   # iid_treeview → id real en BD
        self.pack(fill="both", expand=True)
        self._build()
    ...
    def _cargar_todos(self, filtro=""):
        ...
        # Mostrar número secuencial (1,2,3...) en columna ID
        # El ID real de BD se guarda en _real_ids para editar/eliminar
    ...
    def _get_selected_id(self):
        ...
        if not sel:
            ...
            return None
        # Leer el ID real de BD desde el diccionario interno _real_ids
...
class ConsultaHistorial(tk.Frame):
    ...
    def _build(self):
        ...
        tk.Label(top, text="Consulta de Historial",
                 bg=BG_MAIN, font=FONT_TITLE).pack(side="left")
        make_btn(top, "🗑 Eliminar Registro", self._eliminar_registro,
                 color=COLOR_RED, width=20).pack(side="right", padx=5)
        ...
        res_top = tk.Frame(self, bg=BG_MAIN)
        res_top.pack(fill="x", padx=24, pady=(0,4))
        tk.Label(res_top, text="Historial — cada fila es un par Ingreso/Salida acumulado",
                 bg=BG_MAIN, font=FONT_HEADER).pack(side="left")
        make_btn(res_top, "Limpiar filtros", self._limpiar_filtros,
                 color=COLOR_GRAY, width=16).pack(side="right")
    ...
    def _buscar(self):
        ...
        for idx, (nombre_est, fch, h_entrada, h_salida, tag, est_id, last_reg_id) in enumerate(filas):
            ...
            self.tree.insert("", "end", iid=iid,
                             values=(nombre_est, fch, h_entrada, h_salida, estado_txt),
                             tags=(tag,))
            self._row_meta[iid] = (nombre_est, fch, est_id, last_reg_id)
    ...
    def _eliminar_registro(self):
        ...
        if not seleccion:
            messagebox.showwarning("Sin selección", "Seleccione una fila del historial.")
            return
        iid = seleccion[0]
        nombre_est, fch, est_id, last_reg_id = self._row_meta[iid]
        if not messagebox.askyesno("Confirmar eliminación",
                                   f"¿Eliminar el último movimiento de '{nombre_est}' del {fch}?"):
            return

        conn = get_connection()
        c = conn.cursor()
        if last_reg_id:
            c.execute("DELETE FROM registros WHERE id=?", (last_reg_id,))
        else:
            c.execute("""
            ...
            """, (est_id, fch))
        eliminado = c.rowcount
        conn.commit()
        conn.close()

        if eliminado:
            mostrar_toast(self.winfo_toplevel(), f"🗑 Registro de '{nombre_est}' eliminado.",
                          color=COLOR_RED, duracion=3000)
            self._buscar()
        else:
            messagebox.showinfo("Sin registros", "No se encontró el registro para eliminar.")
...
class GeneracionReportes(tk.Frame):
    ...
    def _build(self):
        ...
        tk.Label(res, text="Reporte — movimientos individuales",
                 bg=BG_CARD, font=FONT_HEADER).pack(anchor="w", pady=(0,10))
        cols = ("ID", "Fecha", "Estudiante", "Tipo", "Hora")
        self.tree = ttk.Treeview(res, columns=cols, show="headings", height=12)
        widths = {"ID":70,"Fecha":160,"Estudiante":340,"Tipo":140,"Hora":140}
        for col in cols:
            self.tree.heading(col,text=col)
    ...
    def _generar(self):
        ...
        for reg_id, fecha, est, tipo, hora in rows:
            ...
            self.tree.insert("","end", values=(reg_id, fecha, est, tipo, hora), tags=(tag,))
        if not rows:
